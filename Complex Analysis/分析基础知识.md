...
### 1.1
#### **定理（幂级数除法）** 设幂级数 $\displaystyle\sum_{n=0}^{\infty} a_n x^n$ 在 $(-R, R)\ (R > 0)$ 内收敛，$a_0 \neq 0$。则存在 $r > 0$，使得幂级数 $\displaystyle\sum_{n=0}^{\infty} b_n x^n$ 在 $(-r, r)$ 内收敛，且$$\left( \sum_{n=0}^{\infty} a_n x^n \right) \left( \sum_{n=0}^{\infty} b_n x^n \right) \equiv 1, \quad \forall\, x \in (-r, r),$$ 或写为$$\frac{1}{a_0 + a_1 x + \cdots + a_n x^n + \cdots} = b_0 + b_1 x + \cdots + b_n x^n + \cdots.$$
